...
\begin{minipage}[t]{0.9\textwidth}
  \centering
 Dada una partición de $M_{J}$, entonces, en ella no podemos repetir ningún sumando 
\end{minipage} 
} 
\end{center} 
Un apunte a tener en cuenta es que, partiendo de una cierta configuración de electrones con el mismo momento angular orbital $l$, en el momento en el que acoplamos el spin, estos electrones pueden tener $j = l + \frac{1}{2}$ o $j = l - \frac{1}{2}$. Por tanto, solo para las combinaciones donde los electrones tengan el mismo $j$, se ha de considerar el principio de exclusión. De lo contrario, los electrones son inequivalentes. En consecuencia, lo primero es plantear las posibles combinaciones de $j$ de los electrones y, para aquella que sea de nuestro interés, aplicar el procedimiento de acoplamiento. \par 
Veamos un ejemplo general. Partamos de una configuración $l^{N}$ con $N < \frac{1}{2} (2l+1)\times(2s+1)$. Acoplando spin y momento angular orbital de cada electrón, los valores que toma $j$ son $j \in \qty{j_{-}, j_{+}} = \qty{l - 1/2, l + 1/2}$ y, por tanto, las posibles combinaciones de $j$  para los $N$ electrones son: 
\begin{align*}
...
\end{align*}
Con estas consideraciones, el procedimiento para obtener los términos j-j es análogo que para el caso Russell--Saunders. Veámoslo con el siguiente ejemplo.
