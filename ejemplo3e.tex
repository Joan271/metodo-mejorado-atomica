...
Consideremos ahora el caso de 3 $e^{-}$ degenerados con $l=2$. La correspondiente configuración electrónica es $\ce{d^{3}}$. Ahora tendremos que considerar tuplas del tipo $\qty(m_{1}^{\sigma_{1}}, m_{2}^{\sigma_{2}},m_{3}^{\sigma_{3}})$. Podremos entonces formar $\mathcal{D} \in  \qty{0,1}$ duplas, que se corresponden con $k \in \qty{3,1}$ sumandos no repetidos. Entonces, mediante la degeneración $G_{k}$ por partición podemos establecer las siguientes reglas:\par
...
\end{align*}
  \end{center}
  \caption{Desarrollo del método para el ejemplo de acoplamiento L-S en $\ce{d^{3}}$ }\label{fig:ejemplo3e}
\end{figure}
